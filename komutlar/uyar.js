...
exports.run = async(client, message, args) => {
  ...
  .setAuthor(`Bu Komutu Kullanmak İçin Yeterli İzine Sahip Değilsin`, message.author.avatarURL)
    ...
    if (!message.member.roles.find("name", "Rol Komutu")) {
        return message.channel.send(' **Bu Komutu Kullanmak için** \*`Rol Komutu*\` **Rolüne Sahip Olman Lazım** ')
  ...
  let modlogkanal = message.guild.channels.find(channel => channel.name === "「📜」uyarılog")
