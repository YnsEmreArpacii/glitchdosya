...
module.exports.run = (client, message, args) => {
  if(!message.member.hasPermission("MUTE_MEMBERS")) return message.reply('Bu komutu kullanabilmek için "Üyeleri Yasakla" iznine sahip olmalısın!').then(m => m.delete(5000));
...
let kullanici = message.mentions.members.first() || message.guild.members.get(args[0])
  ...
  if (kullanici.id === message.author.id) return message.channel.send("Kendinizi susturamazsınız.");
    ...
    let süre = args[1]
  if(!süre) return message.channel.send("Lütfen doğru bir zaman dilimi giriniz. Örneğin: ***ç!seslisustur @kişi 1s/m/h/d sebep**");
  let sebep = args[2]
  if (!sebep) return message.channel.send("Lütfen bir sebep giriniz. Örneğin: ***ç!seslisustur @kişi 1s/m/h/d sebep**");
...
exports.conf = {
    enabled: true,
    guildOnly: true,
    aliases: ["sescezası", "sesli-sustur","seslimute","seslisustur"],
    permLevel: 0
};
